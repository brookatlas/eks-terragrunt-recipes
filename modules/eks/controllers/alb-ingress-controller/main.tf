...
resource "aws_iam_policy" "policy_for_alb_controller" {
...
resource "kubectl_manifest" "aws_load_balancer_controller_service_account" {
  count = var.create ? 1 : 0
  yaml_body = templatefile("alb-controller-service-account.yaml", {
    serviceAccountName=var.service_account_name,
    namespace=var.namespace,
    serviceAccountRoleArn=aws_iam_role.role_for_alb_controller[count.index].arn
  })
}
...
resource "helm_release" "aws-alb-ingress-controller" {
  ...
  depends_on = [ kubectl_manifest.aws_load_balancer_controller_service_account ]
}